...
class PostController extends BackendController
{
    ...
    public function store(PostRequest $request, PostService $PostService)
    {
        ...
        return $this->redirect("Post Created Successfully!", $redirect);
    }
    ...
    public function update(PostRequest $request, PostService $PostService, $id)
    {
        ...
        if (is_string($row)) return $this->throwException($row);
        return $this->redirect("Post Updated Successfully!");
    }
    ...
    public function activeToggle()
    {
        ...
        if (! $post) return response()->json(['failed' => true, 'message' => 'Something is wrong with this post!, Please refresh the page.'], 404);
        $post->update(['active' => !$post->active]);
        return response()->json(['success' => true, 'message' => 'Active status changed successfully!'], 200);
    }
